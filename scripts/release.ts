...
// Run a command, piping `input` to its stdin, with stdout/stderr inherited.
// Used for `gh pr create --body-file -` so PR body content with backticks,
// `$(...)`, etc. is never re-interpreted by the shell.
function shStdin(cmd: string, input: string): void {
  execSync(cmd, { cwd: REPO_ROOT, input, stdio: ['pipe', 'inherit', 'inherit'] });
}
...
function openReleasePR(version: string, entry: string): void {
  ...
  shStdin(`gh pr create --title "chore(release): ${tag}" --body-file -`, body);
