...
```
narrative_nql_execute(query: "CREATE MATERIALIZED VIEW …", data_plane_id: <plane>)
...
narrative_datasets_describe(dataset_ids: [1234], include: ["sample"])
  → sample rows, capped at 100
```

The sample is a point-in-time snapshot capped at 100 rows. It
persists until a new sampling job replaces it, so repeated describes
return the same rows.

## Why coverage checks are queries, not samples

That 100-row cap is smaller than most training sets, which makes a
...
100-row sample zero or one times. Reading "one row for this class" off
...
query so the 100 rows returned are the ones worth reading.
