# Fuzzy name + address variant

When the user runs in **fuzzy match mode** (`--fuzzy` / `--match-mode
fuzzy`, or the interactive "fuzzy name + address" option in Phase 1.5),
the match key stops being a deterministic `graph_edge` identifier and
becomes a **probabilistic composite** of the first name (soundexed) and
the normalized postal address (libpostal). This reference describes the
exact surgery against `assets/workflow.yaml.tmpl`.
...
**edge-extraction** steps (1 and 2) plus the KPI *labels* in step 5.

## The composite key — one atomic key, not two edges
...
A match requires the **same fuzzy first name AND the same address hash**
— the two signals are fused into one key with one `ID_TYPE`, joined with
AND semantics. Do **not** emit separate `soundex_first_name` and
`libpostal` edges; that would match on first-name OR address
independently (every "John" collides; everyone at a shared address
collides) — the opposite of the intent.
...
- **Delimiter rule — the composite MUST use `::`, never `|`.** libpostal
  ...
  composite key ambiguous and the join silently under-matches. The
...
Fuzzy mode needs the two composite signals — `soundex_first_name` (81)
...
- **`standalone-attribute`** — 81 and 326 mapped as their own
  attributes.
- **`graph-edge-json`** — both packed into a fuzzy `graph_edge` (362)
...
reconstructs the composite from those). If a side exposes none of these,
route to `/generate-rosetta-stone-mappings`. The mappings need no
...
The customer fuzzy explode reads the two composite ingredients
...
**How those two ingredients are sourced depends on how the dataset was
...
fuzzy persons / 94,182 overall / 61.8% — SC-62612).
...
DESCRIPTION = 'Step 1/5 of <RUN_SLUG_UPPER>: customer fuzzy edges — explode libpostal address array, composite soundex::hash key.'
EXPIRE = 'P1D'
TAGS = ( '_nio_materialized_view', '_nio_ci_match_report_workflow', '_nio_interactive' )
WRITE_MODE = 'overwrite'
AS
WITH customer_fuzzy AS (
  SELECT DISTINCT
...
SELECT CUSTOMER_PERSON_ID, ID, ID_TYPE FROM customer_fuzzy
```

### Mode B — `graph-edge-json`

Use when the dataset packs both signals into a single fuzzy `graph_edge`
...
CREATE OR REPLACE MATERIALIZED VIEW "<RUN_SLUG_UPPER>_STEP1_CUSTOMER_EDGES"
DISPLAY_NAME = '<RUN_SLUG_UPPER>_STEP1_CUSTOMER_EDGES'
DESCRIPTION = 'Step 1/5 of <RUN_SLUG_UPPER>: customer fuzzy edges from graph_edge target_id JSON, composite soundex::hash key.'
EXPIRE = 'P1D'
TAGS = ( '_nio_materialized_view', '_nio_ci_match_report_workflow', '_nio_interactive' )
WRITE_MODE = 'overwrite'
AS
...
both gated by a `CASE` so only name|address rows emit the fuzzy edge.

## Step 2 — supplier edges (BLOCK, then explode)

**This is the one place fuzzy mode cannot be a naive swap.** The partner
AR is often hundreds of millions of `person_name|postal_address` rows.
`ADDRESS_HASHES` (the libpostal normalizer) is a Snowflake **external
function** — a remote service called in 1,000-row batches — and it
returns **HTTP 500 at high batch volume** (see **SC-61797**). Exploding
it over the full supplier slice will hard-fail the run ~3 minutes in,
and the error is currently swallowed by the platform API.
...
```sql
CREATE OR REPLACE MATERIALIZED VIEW "<RUN_SLUG_UPPER>_STEP2B_SUPPLIER_FUZZY"
DISPLAY_NAME = '<RUN_SLUG_UPPER>_STEP2B_SUPPLIER_FUZZY'
...
template. In fuzzy mode the supplier edges live in
`<RUN_SLUG_LOWER>_step2b_supplier_fuzzy` instead — point step 3's supplier
`FROM` at the 2b table. The join predicate (`ON s.ID = c.ID AND
s.ID_TYPE = c.ID_TYPE`) is unchanged.
...
- `APPROX_COUNT_DISTINCT(ID)` now counts **composite keys**, not
  identities, so `kpi_match_rate` (the *id*-based rate) is a fuzzy-key
  overlap rate, not an identity match rate. **Trust the person-level
  numbers** (`match_totals.persons`, `kpi_match_rate_persons`) for the
  headline.
- The `ID_TYPE` bucket in `customer_identifiers_by_type` /
  ...
  fuzzy channel's contribution.

## Combined mode (deterministic + fuzzy)

The strongest real-world report unions the deterministic email/phone
channel with the fuzzy channel: each side is two MVs (a `graph_edge`
scan **and** a fuzzy explode), `UNION ALL`-ed in step 3, and the report
breaks the match down per `ID_TYPE`. Because the engine is generic over
`(PERSON_ID, ID, ID_TYPE)`, this is purely additive — no new join logic.
A fully worked combined pipeline (customer 41239 vs Audience Acuity and
vs Verisk) lives in the `customer-narrative-demos` repo at
`resources/workflows/{aa,verisk}-fuzzy-match-report.yaml` (and the
target_id-JSON variant at `verisk-fuzzy-match-report-targetid.yaml`). In
the Verisk run the fuzzy channel matched ~30.5K persons alongside email
(~34.4K) and phone (~29.8K) — fuzzy name+address pulled comparable reach
to the deterministic identifiers.
...
2. **Probabilistic.** Soundex collapses distinct names to the same code
   and libpostal maximizes address recall, so fuzzy matches include
   false positives. It trades precision for reach — good for coverage
   estimates, not for a billing-grade identity join.
3. **Tighter key (optional).** To cut false positives, add the family
   name to the composite type:
...
`assets/workflow.fuzzy.yaml.tmpl` the skill selects by file path from the
Phase 1.5 match-mode answer. Land that when the branching outgrows ~30
