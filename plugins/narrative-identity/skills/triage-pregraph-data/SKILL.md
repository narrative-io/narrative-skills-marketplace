...
3. Minimal cuts — you remove bad edges while preserving as much
...
depends on the identifier type, the source system, and what realistic
behavior looks like for the entity being identified.
...
Resolve the source and read its schema (plus sample + stats when
available). For a dataset, run `narrative_datasets_search` then
`narrative_datasets_describe` with `include: ["metadata", "schema",
"sample", "stats"]`. If the source is an access rule, see
[`references/ACCESS_RULES.md`](references/ACCESS_RULES.md) for tool
substitutions and the sample/stats workaround. If neither
`--dataset` nor `--access-rule` was passed and the source type is
ambiguous, ask **one** `AskUserQuestion` to disambiguate before
searching.

Once you have schema and (where applicable) sample / stats, record the
**fully qualified NQL reference** for the source — `company_data.<table>`
for a dataset or `<owning_company_slug>.<rule_name>` for an access
rule. Every downstream phase uses this reference as `<source>`.
...
table that a query writer can translate to NQL. Example:
