...
The snippet covers how to get from a submitted query to the job under
its workflow run, the state machine (`pending` / `running` /
`completed`), and polling cadence; this file documents **what the
`completed` payload actually contains for each job type** and the
second-job sampling dance you have to perform to read rows out of a
...
| --- | --- | --- | --- |
| `materialize-view` | `narrative_nql_execute` with `CREATE MATERIALIZED VIEW "<name>" AS SELECT …`, found via `narrative_jobs_search(workflow_run_id: "<run-uuid>")` | `{dataset_id, snapshot_id, recalculation_id}` | In the **data plane**, on the dataset identified by `dataset_id`. Not on the job. |
...
```
narrative_nql_execute(nql: "CREATE MATERIALIZED VIEW \"my_view\" AS SELECT …")
  → workflow <wf>, run <run>
