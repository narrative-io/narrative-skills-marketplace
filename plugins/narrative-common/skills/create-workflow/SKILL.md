...
Take the chosen example as the skeleton and substitute the user's
values. Hold these invariants while editing:

- `document.dsl` is always `'1.0.0'`. `document.version` is a semver
  string in quotes; bump it manually on every spec change.
...
- Every task `call:` is one of the seven supported tasks:
...
- NQL goes inside `with.nql` as a single string. Wrap multi-line NQL
  ...
  fields end-to-end.
- `datasetName` parameters must match `^[A-Za-z0-9_]{1,256}$`. When
...
- `export.as` is a jq expression. `.` is the current task's output,
  ...
  See `examples/04-data-passing-export-context.yaml`.
- In `${...}` variable expressions, a bare `${expr}` preserves the
  JSON type; inside a string, use jq interpolation `\(expr)`.
...
### "Wrap this NQL as a recurring daily view refresh"
...
### "Build dataset A, then derive B from it"
...
### "Refresh the view, then write a row to the audit log"
...
### "Run customer identity resolution nightly"
...
### "Submit this YAML I already have"
...
- **User asks for parallel tasks, conditional branching, or loops.**
  ...
  be the right shape.
- **`schedule_immediately: true` but no `schedule:` block in the
  spec.** The activate-schedule endpoint will reject this. Either
  drop the flag or add a `schedule.cron` value. Ask which.
- **`--trigger` on a workflow whose first task is destructive.**
  ...
  triggering when the workflow contains an `ExecuteDml` with
  `UPDATE` or `DELETE` unless the user has clearly opted in.
- **`datasetName` casing or invalid characters.** Names must match
  `^[A-Za-z0-9_]{1,256}$`. Reject any name with hyphens, spaces, or
  quotes before drafting — `narrative_workflows_create` will, too.
- **NQL inside the workflow references a dataset on a different
  plane.** The validator cannot catch cross-plane references at
  create time — the run will fail. Surface plane assumptions in step
  6 and verify before submitting.
- **The user pastes a spec with the wrong `dsl` version.** Today the
  only supported value is `'1.0.0'`. Quietly correct it in step 4 and
  flag the change in step 6.
- **Re-submitting the same workflow name+namespace.** The platform
  does not silently overwrite; it returns a conflict. Bump
...
- The skill cannot submit. Stop at step 6: render the YAML and
  explain it, then tell the user explicitly that submission requires
  `narrative-mcp` and offer a `curl` recipe:

  ```
  curl -X POST https://api.narrative.io/workflows \
    -H "Authorization: Bearer $NIO_API_TOKEN" \
    -H "Content-Type: application/x-yaml" \
    --data-binary @<your-workflow>.yaml
  ```

- Never silently degrade. If the spec wasn't submitted because the MCP
  server was down, the final message must say so plainly.

If `narrative-knowledge-base` is unavailable but `narrative-mcp` is:
...
- `assets/examples/*.yaml` — task-shape reference, one example per
  intent. Each file's leading comments document when to use it.
- `assets/templates/workflow-skeleton.yaml` — bare scaffold when no
  example matches the user's case.
- `narrative-knowledge-base` MCP — `/reference/workflows/specification-syntax`
...
- Sibling skills: `/write-nql` for the NQL inside `with.nql` fields,
  ...
  to resolve the canonical `attributeId` of any Rosetta Stone mapping.
