...
## Inputs (from connector-spec.yaml `auth.oauth`)

- `authorize_url`, `token_url`, `me_url`, `redirect_uri`.
- `scopes`, `scope_encoding` (decides the pg column type: `text[]` vs
  `integer[]`).
- `token_response` (which of access/refresh/expires_in the endpoint
  returns → decides the token-table columns).
