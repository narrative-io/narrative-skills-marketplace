...
resolves the fuzzy values into exact ones, and delivers a
**per-skill verdict**: for each downstream skill, either it can run
without guessing or re-asking, or specific named blockers stand in
its way. The overall verdict is **GO** when no blockers remain
anywhere, and **NO-GO otherwise — but NO-GO names which skills are
blocked, and every skill not named runs freely**. What is
...
- **Blocking is skill-scoped.** Every blocker names the skills it
  blocks in its `open_questions` entry's `blocks` list, and it halts
  exactly those skills — a blocker cannot be waived for a skill it
  names, and it never stops a skill it doesn't. A spec can be no-go
  for `/implement-delivery-executor` and simultaneously ready for
  `/scaffold-connector`; say both plainly and suggest the skills
  that are clear to run.
...
- **Blocking** — some downstream skill cannot proceed without the
  answer. The test, applied per skill: would that skill have to
  guess or re-ask? Record the result in the entry's `blocks` list —
  the specific skills that need the answer, not "the pipeline".
  A blocker scoped to no consuming skill is misranked; a blocker
  scoped to every skill is almost always misranked too. Examples:
  unverified identifier attributes block
  `/define-connector-interface` onward; a null `app_id` blocks
  `/register-connector-app`; missing OAuth URLs under
  `auth.model: oauth2` block `/add-connector-oauth`; unknown rate
  limits block `/implement-partner-client`. None of these block
  `/scaffold-connector`, which needs only `slug`, `package_slug`,
  and `delivery.directions`.
- **Non-blocking** — every downstream skill can run; the answer only
  refines behavior later (a partner question about raising an
  already-documented limit, an unsettled display-name wording). The
  entry carries no `blocks` list.

Then:

1. **Write the report.** Save `preflight-report.md` next to the
   spec. Verdict first (**GO** or **NO-GO** on the first line),
   then a per-skill readiness table (skill, ready / blocked, the
   blockers that name it), then the findings as a table (field,
   blocking or non-blocking, which skills it blocks, detail), the
   identifier resolution results, the values enriched, and every
   `open_questions` entry added or updated. The report is the
   artifact a reviewer reads instead of re-deriving the walk.
...
3. **Deliver the verdict.** Go: tell the user the spec is ready and
   ...
   each blocker and the skills it blocks, then say which skills are
   clear and suggest the next one — a no-go for the executor is not
   a reason to sit on a scaffold that's ready. For the skills a
   blocker does name, never soften the answer into "you could
   probably proceed"; those resume by resolving the blocker and
   re-running `/preflight-connector` (or `/probe-partner-api`, when
   the blocker carries a probe).
...
- **`narrative-common:find-attribute` unavailable (no narrative-mcp
  ...
  group becomes a blocking open question scoped to
  `/define-connector-interface` and later, which forces a no-go for
  those skills. Tell the user why the gate holds. An unverified identifier
...
- **`narrative_attribute_create` unavailable** — missing attributes
  ...
  question scoped to the skills that consume the attribute.
