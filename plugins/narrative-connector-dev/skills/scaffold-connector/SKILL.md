...
- **`template-repo`.** Read the manifest at `target.manifest_path`
  ...
  offer to fall back to `reference-clone`, or to help author a
  manifest for the repo (a one-time investment that makes every future
  connector scaffold repeatable).
- **`reference-clone`.** Inspect the reference connector's tree and
  ...
  offer to save the plan into the repo as `connector-scaffold.yaml`
  so future scaffolds skip the inference step.
- **`greenfield`.** Read the runtime profile under
  `references/runtimes/` and ask where the new project directory
  should go. Follow the profile's generation rules, including its rule
  for stubbing platform-facing endpoints whose contract isn't yet
  published (the profile's **platform-contract gate**).
...
- Write back to the spec anything this phase resolved (the final
  ...
  Phase 2 offer to save the confirmed plan as `connector-scaffold.yaml`).
